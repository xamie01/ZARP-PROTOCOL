...
const ERC20_TOKENS = TOKEN_PAIRS.map((p) => ({
  symbol: p.erc20Symbol,
  address: p.erc20Address,
}));

const CONFIDENTIAL_TOKENS = TOKEN_PAIRS.map((p) => ({
...
export default function Wrap() {
  ...
  const selectedTokenData = selectedToken !== null ? currentTokens[selectedToken] : null;
  const outputTokenData = selectedToken !== null ? outputTokens[selectedToken] : null;
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      success(
        `${activeTab === "shield" ? "Tokens Shielded" : "Tokens Unshielded"}`,
        `Successfully ${activeTab === "shield" ? "wrapped" : "unwrapped"} ${amount} ${selectedTokenData?.symbol}`
      );
      setAmount("");
  ...
  const getButtonState = () => {
    ...
    if (selectedToken === null) return { text: "Select Token", disabled: true };
        ...
        <ScrollReveal>
          <div className="bg-white border border-[#E5E7E9] rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-5">
                    ...
                    <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-[#E5E7E9] rounded-xl shadow-lg z-20 max-h-60 overflow-y-auto py-1">
                      {currentTokens.map((token, i) => (
                        ...
                        >
                          ...
                          <span className="text-[#1A1D20] font-medium">
                            {token.symbol}
                          </span>
              ...
              <p className="text-xs text-[#A7ACB3] mt-1.5">
                Balance: —
              </p>
            </div>
            ...
            <div className="flex justify-center -my-3 relative z-10">
              <button
                onClick={() => {
                  setActiveTab(activeTab === "shield" ? "unshield" : "shield");
                  setSelectedToken(null);
                  setAmount("");
                }}
            ...
            <div className="px-6 py-5 bg-[#FAFBFC] border-t border-[#F3F4F5]">
              <label
                className={`text-xs uppercase tracking-wider ${
                  activeTab === "shield" ? "text-[#5D5FEF]" : "text-[#A7ACB3]"
                }`}
              >
              ...
              <p className="text-2xl font-semibold text-[#1A1D20] mt-2">
                {amount || "0.0"}
              </p>
          ...
          <ScrollReveal delay={100}>
            <div className="card-default flex flex-col">
              ...
              <p className="text-sm text-[#656B73] mt-1 leading-relaxed">
                Approve the wrapper, then deposit your ERC-20 tokens. They become
            ...
            {[
              ...
              <ScrollReveal key={step.num} delay={i * 100}>
                <div className="flex gap-4 items-start">
                  <span className="text-2xl font-semibold text-[#FFD100] w-10 flex-shrink-0">
                    {step.num}
                  </span>
                  <div>
