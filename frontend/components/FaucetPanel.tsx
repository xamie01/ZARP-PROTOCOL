  ...
  return (
    <div
      className={cn(
        ...
        "border-[#E5E7E9] bg-white",
        "hover:border-[#FFD100]/60 hover:shadow-[0_4px_16px_rgba(0,0,0,0.06)]",
    ...
    >
      ...
      <div className="p-5 space-y-4">
        {/* Token Symbol and Copy */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-black text-[#FFD100] font-bold text-xs">
              {token.symbol[0]}
            </div>
            <div>
              <h4 className="text-sm font-bold text-[#1A1D20]">{token.symbol}</h4>
              <p className="text-[10px] text-[#A7ACB3]">Decimals: {token.decimals}</p>
            </div>
          </div>
          <button
            onClick={handleCopy}
            title="Copy token address"
            className="rounded-lg border border-[#E5E7E9] bg-[#F8F9FA] p-1.5 text-[#A7ACB3] hover:text-black hover:border-[#CDD0D4] transition-all"
          >
            {copied ? (
        ...
        {/* Address and Balance */}
        <div className="space-y-2 rounded-xl bg-[#F3F4F5] p-3.5 border border-[#E5E7E9]">
          <div className="flex items-center justify-between text-[11px]">
            <span className="text-[#A7ACB3]">Address:</span>
            <span className="font-mono text-[#4D535A]">{truncateAddress(token.address, 6)}</span>
          </div>
          <div className="flex items-center justify-between text-[11px] border-t border-[#E5E7E9] pt-2">
            <span className="text-[#A7ACB3]">Your Balance:</span>
            <span className="font-mono font-semibold text-[#1A1D20]">
        ...
        {/* Cooldown Timer */}
        {timeLeft > 0 && (
          <div className="flex items-center justify-center gap-1.5 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs font-semibold text-amber-600 font-mono">
        ...
        <button
          ...
          className={cn(
            ...
            canRequest && !isSequencing
              ? "btn-primary"
              : "border border-[#E5E7E9] bg-[#F3F4F5] text-[#A7ACB3] cursor-not-allowed"
          )}
        >
          {isLoading ? (
      ...
      {/* Transaction & Error Footer */}
      {(tx || error) && (
        <div className="border-t border-[#F3F4F5] bg-[#F8F9FA] p-3 text-[11px]">
          {tx && (
            <div className="flex items-center justify-between text-[#2ECC71]">
              <span className="flex items-center gap-1">
                <CheckCircle2 className="h-3.5 w-3.5 shrink-0" /> Minted!
              </span>
              <a
                href={`https://sepolia.etherscan.io/tx/${tx}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-0.5 hover:underline font-mono text-[10px] text-[#5D5FEF]"
              >
          ...
          {error && (
            <div className="text-[#E74C3C] flex items-start gap-1">
...
export function FaucetPanel() {
  ...
  const balances = CTOKEN_MOCKS.map((token, index) => {
    ...
    return rawBalance !== undefined ? formatTokenAmount(rawBalance, token.decimals) : "0";
  });
  ...
  const handleRequestAll = async () => {
    ...
    try {
      for (let i = 0; i < CTOKEN_MOCKS.length; i++) {
        ...
        if (now - lastRequestTime < COOLDOWN_24H) continue;
  ...
  useEffect(() => {
    ...
    for (const token of CTOKEN_MOCKS) {
      ...
      const lastRequestVal = localStorage.getItem(storageKey);
      const lastRequestTime = lastRequestVal ? parseInt(lastRequestVal, 10) : 0;
      if (Date.now() - lastRequestTime >= COOLDOWN_24H) count++;
    }
  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#CDD0D4] bg-[#F8F9FA] p-12 text-center">
        ...
        <p className="mt-1 text-xs text-[#656B73]">Connect your wallet to use the testnet faucet dashboard</p>
      </div>
    );
  }
  ...
  return (
    <div id="faucet-panel" className="animate-fade-in space-y-6">
      {/* Dashboard Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-2xl border border-[#E5E7E9] bg-white p-6">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-black text-[#FFD100] shadow-sm">
            <Coins className="h-5 w-5" />
          </div>
          <div>
            <h3 className="text-base font-bold text-[#1A1D20]">Faucet Dashboard</h3>
        ...
        <div className="flex items-center gap-2">
          {sequenceStatus && (
            <div className="hidden lg:flex items-center gap-1.5 rounded-lg bg-[#FFD100]/10 border border-[#FFD100]/30 px-3 py-1.5 text-xs text-black font-medium">
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
      ...
      {sequenceStatus && (
        <div className="lg:hidden flex items-center gap-2 rounded-xl bg-[#FFD100]/10 border border-[#FFD100]/30 p-3 text-xs text-black font-medium">
      ...
      <p className="text-center text-[10px] text-[#A7ACB3]">
