...
1. **Load `.claude/skills/fhevm-skill/SKILL.md`** and follow its workflow-selection logic. The skill auto-activates on the triggers above. Do not skip it.
2. **Load `.claude/skills/fhevm-skill/references/anti-patterns.md`**. It lists 62 verified mistakes AI agents repeat when writing FHEVM code. Skipping this is the single biggest cause of broken output.
...
- Full workflows (Generate / Review / Test / Deploy / Audit / Frontend on legacy SDK / Frontend on new `@zama-fhe/sdk@^3` Token API): `.claude/skills/fhevm-skill/SKILL.md`
- Types, operations, HCU costs: `.claude/skills/fhevm-skill/references/types-operations.md`
- ACL patterns: `.claude/skills/fhevm-skill/references/acl.md`
- Input handling and decryption flows (legacy `@zama-fhe/relayer-sdk` primitives): `.claude/skills/fhevm-skill/references/inputs-decryption.md`
- ERC-7984 confidential tokens (Solidity layer): `.claude/skills/fhevm-skill/references/erc7984.md`
- Hardhat testing: `.claude/skills/fhevm-skill/references/testing.md`
- Frontend / relayer SDK (incl. COOP/COEP, mainnet API-key auth, browser bundler quirks): `.claude/skills/fhevm-skill/references/frontend.md`
- Deployment (Hardhat only, Path A recommended, Path B uses `hardhat-deploy`): `.claude/skills/fhevm-skill/references/deployment.md`
- Audit patterns P1-P32: `.claude/skills/fhevm-skill/references/fhe-vulnerabilities.md`
- General Solidity attack vectors G1-G29: `.claude/skills/fhevm-skill/references/solidity-vulnerabilities.md`
- Contract templates: `.claude/skills/fhevm-skill/templates/`
- Parallel audit modules: `.claude/skills/fhevm-skill/review-modules/`

### New SDK (`@zama-fhe/sdk@^3.0.0` and `@zama-fhe/react-sdk@^3.0.0`)

- Overview, package map, `ZamaSDK` constructor, network presets: `.claude/skills/fhevm-skill/references/zama-sdk-overview.md`
- Auth (backend proxy + 3 `auth` shapes + CSRF), `RelayerWeb` / `RelayerNode` / `RelayerCleartext`, signer adapters, storage (IndexedDB / Memory / AsyncLocal / ChromeSession), web-extension setup: `.claude/skills/fhevm-skill/references/zama-sdk-auth-storage.md`
- `Token` / `ReadonlyToken`, shield / unshield (resumable two-phase) / `confidentialTransfer` / balance: `.claude/skills/fhevm-skill/references/zama-sdk-tokens.md`
- Session model (keypairTTL / sessionTTL), `allow` / `userDecrypt` / `revokeSession`, full delegation API: `.claude/skills/fhevm-skill/references/zama-sdk-session.md`
- `ZamaProvider`, 59 React hooks grouped by category, Next.js SSR, Vite, web extensions, `zamaQueryKeys` cache control: `.claude/skills/fhevm-skill/references/zama-sdk-react.md`
- Error taxonomy (26 classes: `ZamaError` base + 25 subclasses), `matchZamaError` patterns, common recovery flows: `.claude/skills/fhevm-skill/references/zama-sdk-errors.md`
- Activity feeds + event decoders (token + ACL), `WrappersRegistry`, contract builders, operator approvals deep dive, FHE artifact cache: `.claude/skills/fhevm-skill/references/zama-sdk-activity.md`
